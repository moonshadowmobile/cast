...
function printCertData(data) {
  ...
  for (key in data) {
    if (data.hasOwnProperty(key)) {
      util.log(sprintf('    %s: %s', key, data[key]));
    }
  }
}
...
function handleCommand(args, parser, callback) {
  ...
  if (isHttps) {
    ...
    calls.push(function(remotes, certInfo, callback) {
      ...
      util.log('Certificate information');
      util.log('  Subject:');
      printCertData(certInfo['subject']);
      util.log('  Issuer:');
      printCertData(certInfo['issuer']);
      util.log(sprintf('  Valid to: %s', certInfo['valid_to']));
      util.log(sprintf('  Fingerprint: %s', certInfo['fingerprint']));
      util.log('');
