...
parser.on('print', function(value) {
    util.log('PRINT: ' + (value || 'No message entered'));
});
...
parser.on('help', function() {
    util.log(parser.toString());
    print_summary = false;
});

// Set a default handler
parser.on('*', function(opt, value) {
    util.log('wild handler for ' + opt + ', value=' + value);
});
...
if(print_summary) {
    util.log("First non-switch argument is: " + first_arg);
    
    // Output all files that was included.
    util.log("No of files to include: " + options.files.length);
    for(var i = 0; i < options.files.length; i++) {
        util.log("File [" + (i + 1) + "]:" + options.files[i]);
    }

    // Is debug-mode enabled?
    util.log("Debug mode is set to: " + options.debug);

    util.log("Number value is: " + options.number);
    util.log("Date value is: " + options.date);
}