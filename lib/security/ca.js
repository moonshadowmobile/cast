...
util.inherits(SigningRequest, jobs.DirectoryResource);
  ...
  return [
    swiz.struct.Obj(
      'SigningRequest',
      {
        'fields': [
          F('name', {'src': 'name'}),
          F('csr', {'src': 'getRequestText'}),
          F('cert', {'src': 'getCertificateText'})
        ]
      }
    )
  ];
...
SigningRequest.prototype.getCertificateText = function(callback) {
  fs.readFile(this.getCRTPath(), 'utf8', function(err, text) {
    ...
    if (err && err.code === errCode.ENOENT.code) {
      err = undefined;
      text = null;
    }
    callback(err, text);
...
util.inherits(SigningRequestManager, jobs.ResourceManager);
